...
```bash
# Repair duplicate logical native-session rows
ais changelog repair-native-sync

# Remove already-synced explicit Codex subagent-derived rows
...
| `claude` | Default: `opus[1m]` with 8192 thinking tokens | More detailed analysis |

**Note:** When using the `claude` evaluator, `ai-code-sessions` runs Claude Code in headless mode with MCP servers disabled (via `--strict-mcp-config` and an empty `--mcp-config`) to keep evaluation fast and deterministic. Prompt payloads are delivered through stdin (not argv), written to `.tmp/changelog-eval/` during evaluation, and archived to `.archive/changelog-eval/` only on failed attempts. Budget-mode retry is a last-resort fallback after a failed full-prompt attempt.
...
export CTX_CHANGELOG_MODEL=""  # Optional; blank uses the default `opus[1m]`
```
...
[changelog]
evaluator = "claude"
model = ""  # Optional; blank uses the default `opus[1m]`
...
Report and optionally repair duplicate logical native-session rows previously written by sync.

```bash
ais changelog repair-native-sync
ais changelog repair-native-sync --project-root "$(git rev-parse --show-toplevel)" --actor "username"
ais changelog repair-native-sync --apply
```

### `ais changelog repair-subagent-sync`

Report and optionally remove explicit Codex subagent-derived sync rows from existing changelog files.

```bash
ais changelog repair-subagent-sync
ais changelog repair-subagent-sync --project-root "$(git rev-parse --show-toplevel)" --actor "username"
