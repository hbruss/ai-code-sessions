...
- For Codex GitHub reviews in this repo, the clean signal is the `chatgpt-codex-connector[bot]` thumbs-up reaction on the PR body; an eyes reaction means still processing, and a review comment alone is not the full gate.
- For CodeRabbit reviews, inspect the submitted review body as well as inline pull-request comments. Outside-diff findings may be embedded in the review body and will not appear from `pulls/{n}/comments`.
