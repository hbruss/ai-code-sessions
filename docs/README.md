...
It’s designed to work with your existing `ais ctx` workflow:

- You start a session with a human-friendly label (e.g. `ais ctx "Fix login bug" --codex`)
- You work normally (with full terminal colors/UI intact)
- When you quit, `ais ctx` writes a per-repo session directory (timestamp + label) and auto-generates:
...
- What gets generated and where (`ais ctx` output layout)
...
- `docs/ctx.md` — how `ais ctx` works (colors preserved + auto export)
- `docs/cli.md` — CLI commands and examples
- `docs/config.md` — global + per-repo configuration
