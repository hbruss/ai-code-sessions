...
def test_sync_shaped_changelog_entry_uses_session_start_and_null_transcript_fields(monkeypatch, tmp_path: Path):
    project_root = tmp_path / "repo"
    project_root.mkdir()
    ...
    assert entry["source"]["identity"] == ai_code_sessions._canonical_session_identity_for_source(
        tool="codex",
        source_jsonl=source_jsonl,
        start=start,
        end=end,
